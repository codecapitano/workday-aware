...
Pass custom values to `setup` with `--timezone`, `--work-days`, `--end-of-day`, `--wrap-up-minutes`, and `--meaningful-work-minutes`; preview first, then repeat the same command with `--confirm`.
